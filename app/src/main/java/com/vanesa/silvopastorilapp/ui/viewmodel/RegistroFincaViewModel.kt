...
class RegistroFincaViewModel : ViewModel() {
    // Departamentos
    ...
    var postFincaUseCase = PostFincaUseCase()
    ...
    fun postFinca(fincaDTO: FincaDTO) {
        viewModelScope.launch {
            isLoading.postValue(true)
            val result: DataResponse<String> = postFincaUseCase(fincaDTO)
            when (result.status) {
                "success" -> {
                    message.postValue(result.message)
                    if (result.data[0] == "created") {
                        message.postValue("Finca creada de manera exitosa")
                        isLoading.postValue(false)
